...
export default function UserProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'symptoms' | 'history'>('overview');
    ...
    <div style={{ 
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          {(['overview', 'symptoms', 'history'] as const).map(tab => (
            <button
              key={tab}
            ...
            {userSymptoms.length === 0 ? (
              <div style={{
                ...
                <p style={{ color: 'var(--muted)' }}>
                  Start tracking symptoms to see your health trends over time.
                </p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    <div style={{ fontSize: '2rem', color: 'var(--accent)', marginBottom: '8px' }}>
                      {userSymptoms.length}
                    </div>
                    <div style={{ fontWeight: '500' }}>Total Symptoms</div>
                  </div>
                  ...
                  <div style={{
                    ...
                    <div style={{ fontSize: '2rem', color: 'var(--accent2)', marginBottom: '8px' }}>
                      {(userSymptoms.reduce((sum, s) => sum + s.severity, 0) / userSymptoms.length).toFixed(1)}
                    </div>
                    <div style={{ fontWeight: '500' }}>Average Severity</div>
                  </div>
                  ...
                  <div style={{
                    ...
                    <div style={{ fontSize: '2rem', color: '#f59e0b', marginBottom: '8px' }}>
                      {Math.max(...Object.values(SYMPTOM_CATEGORIES).map(cat => 
                        userSymptoms.filter(s => s.category === Object.keys(SYMPTOM_CATEGORIES).find(key => 
                          SYMPTOM_CATEGORIES[key as keyof typeof SYMPTOM_CATEGORIES] === cat
                        )).length
                      ))}
                    </div>
                    <div style={{ fontWeight: '500' }}>Most Affected Area</div>
                  </div>
                </div>
                    ...
                    {userSymptoms
                      ...
                      <div
                        ...
                        style={{
                          ...
                          borderRadius: '8px',
                          borderLeft: `4px solid ${symptom.categoryColor}`
                        }}
                      >
                        <div style={{
                          minWidth: '80px',
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: '500' }}>{symptom.name}</div>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {Object.entries(groupedSymptoms).map(([categoryName, symptoms]) => {
                      const categoryData = SYMPTOM_CATEGORIES[categoryName as keyof typeof SYMPTOM_CATEGORIES];
